...
func (b *WhereExpressionBuilder) Build() *cmexpr.SelectExpression {
	...
	for _, v := range b.and {
		log(s, v, nil, cmexpr.LogicalOperatorAnd)
	}

	for _, v := range b.or {
		log(s, v, nil, cmexpr.LogicalOperatorOr)
	}

	if b.not != nil {
		log(s, b.not, nil, cmexpr.LogicalOperatorNot)
	}
...
func log(
	...
	for _, and := range l.and {
		log(s, and, scp, cmexpr.LogicalOperatorAnd)
	}

	for _, or := range l.or {
		log(s, or, scp, cmexpr.LogicalOperatorOr)
	}

	if l.not != nil {
		log(s, l.not, scp, cmexpr.LogicalOperatorNot)
	}
