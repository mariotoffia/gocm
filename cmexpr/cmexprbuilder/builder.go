...
func (b *WhereExpressionBuilder) Build() *cmexpr.SelectExpression {
	...
	if len(b.propertyExpressions) == 1 {
		...
		s.Where.AndExpressions = append(s.Where.AndExpressions, expr)
	}
	...
	if len(b.or) > 0 {
		...
		for _, v := range b.or {
			...
			s.Where.OrExpressions = append(s.Where.OrExpressions, expr)
		}
	}
	...
	if b.not != nil {
		...
		s.Where.NotExpression = &cmexpr.NotExpression{
